...
const main = async () => {
  ...
  let deployConfig =
    process.env.HARDHAT_NETWORK === "kovan" || config.defaultNetwork === "kovan"
      ? kovanConfig
      : mainnetConfig;

  const aaveApe = await deploy("LendingPool", [
    deployConfig.lendingPoolAddressesProvider,
    deployConfig.uniswapRouterAddress,
  ]);
